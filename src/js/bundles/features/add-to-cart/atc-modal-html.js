...
const renderComparePrice = showCompare => {
  ...
  if (showCompare) {
    return (
      ...
      "<s className='bm-atc-modal__pricing-price-compare text-muted d-block bm-fonts fs-8 mm-modal__readable'>" +
...
const atcModalPricing = item => {
  ...
  return (
    ...
    `<span class='bm-atc-modal__pricing-price mm-modal__readable d-block bm-colors-color-grey-50 bm-fonts fs-8${renderItemPriceClasses(
      showCompare
    )}` +
    "'>" +
...
const atcModalDetails = item => {
  return (
    ...
    `<h6 class='bm-title mt-0 mb-3 fs-6 mm-modal__readable'>${item.title}</h6>` +
...
const atcModalActions = () => {
  return (
    ...
    "<span class='bm-btn__content d-block mm-modal__readable'>View Bag</span>" +
    "</span>" +
    "</a>" +
    "</div>" +
    ...
    "<span class='bm-btn__wrapper d-flex align-items-center justify-content-center'>" +
    "<span class='bm-btn__content d-block mm-modal__readable'>Checkout</span>" +
    "</span>" +
    "</a>" +
    "</div>" +
    "</div>"
...
export const addedHtml = () => {
  const bmApi = bmApiObj();
  return (
    "<span class='d-block me-1'>" +
    "<i class='bm-icon bm-icon__icon-star d-block'>" +
    "<svg xmlns='http://www.w3.org/2000/svg' width='16' height='16' fill='currentColor' class='bi bi-check-circle-fill d-block bm-icon__size--regular bm-colors-color-brown mm-modal__readable' viewBox='0 0 16 16'>" +
    "<path d='M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zm-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z'></path>" +
    "</svg>" +
    "</i>" +
    "</span>" +
    "<span class='d-block mm-modal__readable'>" +
...
const atcModalMessages = messages => {
  ...
  const messageItems = messages
    .map(message => {
      return "<p class='mt-0 mb-3 fs-6 mm-modal__readable'>" + message + "</p>";
    })
    .join("");

  return (
    "<div class='bm-atc-modal__messages'>" +
    "<p class='mt-0 mb-3 fs-6 mm-modal__readable'>" +
    messageItems +
    "</p>" +
    "</div>"
  );
...
export const messageHtml = items => {
  const messages = handleAtcMessageFlow(items);

  return atcModalMessages(messages);
};

export const atcModalItemsHtml = items => {
